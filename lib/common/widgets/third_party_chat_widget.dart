...
class ThirdPartyChatWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<SplashProvider>(builder: (context, splashProvider, _) {
      return !(splashProvider.configModel?.maintenanceMode == 1) &&
              (splashProvider.configModel?.companyWhatsapp != null)
          ? InkWell(
              onTap: () async {
                final String? whatsapp =
                    splashProvider.configModel!.companyWhatsapp;
                ...
                if (await canLaunchUrl(whatsappMobile)) {
                  ...
                  await launchUrl(
                      Uri.parse(
                          "https://web.whatsapp.com/send?phone=$whatsapp"),
                      mode: LaunchMode.externalApplication);
                }
              },
              child: Container(
