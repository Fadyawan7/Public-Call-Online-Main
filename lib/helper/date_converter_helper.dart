...
class DateConverterHelper {
  ...
  static String chatTimeOnly(String? dateTime, BuildContext context) {
    if (dateTime == null || dateTime.trim().isEmpty) {
      return '';
    }

    final String value = dateTime.trim();
    final String normalized =
        value.contains(' ') ? value.replaceFirst(' ', 'T') : value;
    final DateTime? parsedDate = DateTime.tryParse(normalized);

    if (parsedDate != null) {
      return dateToTimeOnly(parsedDate.toLocal(), context);
    }

    final RegExpMatch? timeMatch =
        RegExp(r'(\d{1,2}:\d{2})(?::\d{2})?').firstMatch(value);

    return timeMatch?.group(1) ?? value;
  }
  ...
  static DateTime isoStringToLocalDate(String dateTime, {String? format}) {
    return DateFormat(format ?? 'yyyy-MM-ddTHH:mm:ss.SSS')
        .parse(dateTime, true)
        .toLocal();
  }
  ...
  static bool isAvailable(String start, String end, {DateTime? time}) {
    ...
    if (time != null) {
      ...
      currentTime =
          Provider.of<SplashProvider>(Get.context!, listen: false).currentTime;
    }
  ...
  static String getEstimateTime(Duration duration, BuildContext context) {
    return duration.inDays > 1
        ...
        : duration.inHours > 1
            ? '${duration.inHours} ${getTranslated('hour', context)}'
            : duration.inMinutes > 5
                ? '${duration.inMinutes - 5} - ${duration.inMinutes} ${getTranslated('min', context)}'
