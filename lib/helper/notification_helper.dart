  ...
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
        ...
        try{
          if(notificationResponse.payload!.isNotEmpty) {
            if(payload.orderId != null && payload.orderId != 'null' && payload.orderId != '') {
              RouterHelper.getBookingDetailsRoute(payload.orderId);
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message)async {
      ...
      if(message.data['type'] == 'maintenance'){
        ...
        if(splashProvider.configModel?.maintenanceMode == 1) {
          RouterHelper.getMaintainRoute(action: RouteAction.pushNamedAndRemoveUntil);
        }
        if(splashProvider.configModel?.maintenanceMode == 0
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async{
      ...
      if(message.data['type'] == 'maintenance'){
        ...
        if(splashProvider.configModel?.maintenanceMode == 0 && ModalRoute.of(Get.context!)?.settings.name == RouterHelper.maintain){
          RouterHelper.getMaintainRoute(action: RouteAction.pushNamedAndRemoveUntil);
        }
      }
      ...
      try{
        if(message.notification!.titleLocKey != null && message.notification!.titleLocKey!.isNotEmpty) {
          RouterHelper.getBookingDetailsRoute(message.notification!.titleLocKey);
        }
      }catch (e) {
        debugPrint('error ===> $e');
      }
