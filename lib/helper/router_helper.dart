...
class RouterHelper {
  ...
  static const String freelancerBookingDetailsScreen =
      '/freelancer-booking-details';
  ...
  static const String freelancerPortfolioListScreen =
      '/freelancer-portfolio-list';
  static const String freelancerPortfolioAddScreen =
  ...
  static String getBookingDetailsRoute(String? id, {RouteAction? action}) =>
      _navigateRoute('$bookingDetailsScreen?id=$id');
  static String getFreelancerBookingDetailsRoute(String? id) =>
      _navigateRoute('$freelancerBookingDetailsScreen?id=$id');

  static String getUserDetailsRoute(
      {BookingDetailsModel? bookingDetail, RouteAction? action}) {
    String bookingDetailData =
        base64Url.encode(utf8.encode(jsonEncode(bookingDetail?.toJson())));

    return _navigateRoute(
        '$userDetailsScreen?bookingDetailData=$bookingDetailData',
        route: action);
  }

  static String getChatRoute(
      {FreelancerModel? freelancer, RouteAction? action}) {
    String freelancerData =
        base64Url.encode(utf8.encode(jsonEncode(freelancer?.toJson())));

    return _navigateRoute('$chatScreen?freelancer=$freelancerData',
        route: action);
  }
  ...
  static String getCheckoutRoute(
    ...
    return _navigateRoute(
        '$checkoutScreen?amount=$amount0&page=$page&&code=$code${isCutlery ? '&cutlery=1' : ''}');
  }
  ...
  static String getRateReviewListRoute(String? id) =>
      _navigateRoute('$rateReviewListScreen?id=$id');
  static String getChangePasswordRoute() =>
      _navigateRoute(changePasswordScreen);

  static String getSubmitRateReviewRoute(
          {required bookingId, required takerId}) =>
      _navigateRoute('$submitRateScreen?id=$bookingId&taker_id=$takerId');
  static String getProfileRoute(String page,
          {RouteAction action = RouteAction.push}) =>
      _navigateRoute('$profileScreen?page=$page', route: action);
  static String getFreelancerBookingRoute(
          {RouteAction action = RouteAction.push}) =>
      _navigateRoute(freelancerBookingScreen, route: action);

  static String getApplyFreelancerRoute(
          {RouteAction action = RouteAction.push}) =>
      _navigateRoute(applyFreelancerScreen, route: action);
  static String getFreelancerPortfolioListRoute(
          {RouteAction action = RouteAction.push}) =>
      _navigateRoute(freelancerPortfolioListScreen, route: action);
  static String getFreelancerPortfolioAddRoute(
          {RouteAction action = RouteAction.push}) =>
      _navigateRoute(freelancerPortfolioAddScreen, route: action);

  static String getBookingDateSlotRoute(String freelancerId) =>
      _navigateRoute('$bookingDateSlotScreen?freelancer_id=$freelancerId',
          route: RouteAction.push);

  static String getFreelancerDetailRoute(
      {FreelancerModel? freelancerModel,
      RouteAction action = RouteAction.push}) {
    String freelancerModel0 =
        base64Url.encode(utf8.encode(jsonEncode(freelancerModel!.toJson())));
    return _navigateRoute(
        '$freelancerDetailScreen?freelancerDetail=$freelancerModel0',
        route: action);
  }
  ...
  static String getOrderSuccessScreen(String? status, String? message) {
    return _navigateRoute(
        '$orderSuccessScreen?status=$status&message=$message');
  }
    ...
    routes: [
            ...
            return _routeHandler(
                ...
                DashboardScreen(
                  pageIndex: state.uri.queryParameters['page'] == 'booking'
                      ? 1
                      : state.uri.queryParameters['page'] == 'freelancer'
                          ? 2
      ...
      GoRoute(
          ...
          builder: (context, state) {
            ChatModel? chat;
            try {
              chat = ChatModel.fromJson(jsonDecode(utf8.decode(base64Url.decode(
                  '${state.uri.queryParameters['chat']?.replaceAll(' ', '+')}'))));
            } catch (error) {
      ...
      GoRoute(
          path: rateReviewListScreen,
          builder: (context, state) => _routeHandler(
              context,
              path: _getPath(state),
              RateReviewListScreen(
                freelancerId: int.parse(state.uri.queryParameters['id'] ?? ''),
              ))),
      GoRoute(
          path: bookingDateSlotScreen,
          builder: (context, state) => _routeHandler(
              context,
              path: _getPath(state),
              BookingDateSlotScreen(
                freelancerId: state.uri.queryParameters['freelancer_id'],
              ))),
      GoRoute(
          path: notificationScreen,
          builder: (context, state) => _routeHandler(
              context, path: _getPath(state), const NotificationScreen())),
      GoRoute(
          path: chatScreen,
          builder: (context, state) =>
      ...
      GoRoute(
          ...
          builder: (context, state) => _routeHandler(
              context, path: _getPath(state), const ChangePasswordScreen())),
      GoRoute(
          path: profileScreen,
          builder: (context, state) => _routeHandler(
              context,
      ...
      GoRoute(
          path: applyFreelancerScreen,
          builder: (context, state) => _routeHandler(
              context, path: _getPath(state), const ApplyFreelancerScreen())),
      GoRoute(
          path: freelancerPortfolioListScreen,
          builder: (context, state) => _routeHandler(
              context,
              path: _getPath(state),
              const FreelancerPortfolioScreen())),
      GoRoute(
          path: freelancerPortfolioAddScreen,
          builder: (context, state) => _routeHandler(
              context,
              path: _getPath(state),
              const FreelancerPortfolioAddScreen())),
      GoRoute(
          path: addressScreen,
          builder: (context, state) => _routeHandler(
      ...
      GoRoute(
          ...
          builder: (context, state) {
            ...
            try {
              bookingDetailsModel = BookingDetailsModel.fromJson(jsonDecode(
                  utf8.decode(base64Url.decode(
                      '${state.uri.queryParameters['bookingDetailData']?.replaceAll(' ', '+')}'))));
      ...
      GoRoute(
          path: freelancerBookingDetailsScreen,
          builder: (context, state) => _routeHandler(
              context,
              path: _getPath(state),
              FreelancerBookingDetailsScreen(
                bookingId: int.parse(state.uri.queryParameters['id'] ?? ''),
              ))),
    ],
  );
