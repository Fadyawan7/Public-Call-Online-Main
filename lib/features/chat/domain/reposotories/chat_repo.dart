...
class ChatRepo {
  ...
  Future<ApiResponseModel> startNewChat(int userId) async {
    try {
      ...
      data['_method'] = 'post';
      final response =
          await dioClient!.post(AppConstants.startNewChatUrl, data: data);
  ...
  Future<ApiResponseModel> getConversationList(int chatId) async {
    try {
      final response = await dioClient!
          .get('${AppConstants.getConversationsUrl}$chatId/messages');
  ...
  Future<http.StreamedResponse> sendMessage(
      String message, File? file, int? chatId, String token) async {
  ...
  Future<ApiResponseModel> deleteChat(int chatId) async {
    try {
      ...
      data['_method'] = 'delete';
      final response = await dioClient!
          .post('${AppConstants.deleteUrl}/$chatId', data: data);
