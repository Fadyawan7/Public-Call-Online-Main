...
class HappyProfileScreen extends StatefulWidget {
  final bool fromSplash;

  const HappyProfileScreen({super.key, required this.fromSplash});

  @override
  State<HappyProfileScreen> createState() => _HappyProfileScreenState();
}

class _HappyProfileScreenState extends State<HappyProfileScreen> {
  ...
  Future<void> _updateProfile(ProfileProvider profileProvider) async {
    if (profileFormKey.currentState?.validate() ?? false) {
      ...
      } else if (profileProvider.selectedCityID! == -1 &&
          profileProvider.userInfoModel!.cityId == -1) {
