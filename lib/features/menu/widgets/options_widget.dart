...
class _OptionsWidgetState extends State<OptionsWidget> {
  ...
  bool _categoryDirectoryRequested = false;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    _loadFreelancerCategoriesIfNeeded();
    _loadCategoryDirectoryIfNeeded();
  }

  void _loadCategoryDirectoryIfNeeded() {
    if (_categoryDirectoryRequested) {
      return;
    }
    _categoryDirectoryRequested = true;
  ...
  /// Resolves the freelancer's category chips.
  ///
  /// [directory] is CategoryProvider.categoryList, passed in explicitly by
  /// a Consumer<CategoryProvider> in build() so this stays reactive: once
  /// the directory finishes loading and calls notifyListeners(), the
  /// Consumer rebuilds and this re-runs with the populated list - no
  /// manual setState wiring needed here.
  List<String> _buildCategoryItems(List<OnlyCategoryModel>? directory) {
    ...
    // category the freelancer belongs to. The profile/getUserInfo response
    // (ProfileProvider.userInfoModel) is the confirmed-accurate source for
    // the full list of ids, so prefer that over the freelancer-details one.
    final profileProvider =
        Provider.of<ProfileProvider>(context, listen: false);
    final categoryIds =
        (profileProvider.userInfoModel?.categoryIds?.isNotEmpty ?? false)
            ? profileProvider.userInfoModel!.categoryIds
            : _freelancer?.category_ids;
  ...
  Widget build(BuildContext context) {
    ...
    final bool isFreelancerAccount =
        Provider.of<ProfileProvider>(context, listen: false)
                .userInfoModel
                ?.userType ==
            'freelancer';

    // Listening (not listen: false) here is what makes the chips appear
    // automatically once CategoryProvider.getCategoryList() finishes and
    // calls notifyListeners() - no manual setState wiring needed.
    return Consumer<CategoryProvider>(builder: (context, categoryProvider, _) {
      final categoryItems = _buildCategoryItems(categoryProvider.categoryList);
      final bool isLoadingCategorySection = _isLoadingFreelancer ||
          (isFreelancerAccount &&
          ...
          child: Ink(
            ...
            child: Column(children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                if (categoryItems.isEmpty && !isLoadingCategorySection)
                  SizedBox.shrink()
                else
                  Column(
                        ...
                        child: Column(
                          ...
                          children: [
                            Padding(
                              ...
                              child: isLoadingCategorySection
                                  ? const Center(
                                      child: SizedBox(
                                        height: 18,
                                        width: 18,
                        ...
                        Consumer<ProfileProvider>(
                          builder: (context, profileProvider, child) {
                            String? applicationStatus = profileProvider
                                    .userInfoModel?.freelancerRequestStatus ??
                                '';
                            String? applicationStatusNote = profileProvider
                                    .userInfoModel?.freelancerRequestNote ??
                                '';
                            ...
                            return PortionWidget(
                              ...
                              suffix:
                                  (applicationStatus == 'needed_more_data' ||
              ...
              const SizedBox(height: Dimensions.paddingSizeExtraSmall),
              Text('${getTranslated('v', context)} ${AppConstants.appVersion}',
                  style: rubikRegular.copyWith(
                    color: Theme.of(context)
                        .textTheme
                        .titleMedium
